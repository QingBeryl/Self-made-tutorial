...
![image-20260704185451200](static/docs/Git使用教程/images/image-20260704185451200.png)

- [本地分支名] :[远端分支名]：若两个名称相同，那么可以只写一个，一般情况下都是相同的；
- [远端名称]：添加远程仓库时设置的，默认为`origin`,远程仓库可以有多个；
- -f：强制推送覆盖远程仓库中的代码。

说明：首次推送时会需要验证身份，输入Github的账号密码即可，如图所示：

![image-20260704200537980](static/docs/Git使用教程/images/image-20260704200537980.png)

### 7. 本地分支与远程分支的关联关系

在`git push`命令后添加以下参数，可以建立本地分支与远程分支的关系，使用后后续推送可省略`git push`后的所有内容:

```bash
git push --set-upstream [远端名称] [本地分支名]
```

使用如下命令可以查看本地分支与远程分支的付对应关系：
...
![image-20260705111726827](static/docs/Git使用教程/images/image-20260705111726827.png)

### 8. 从远程仓库克隆

使用如下命令，可以将已有的一个远程仓库克隆到本地：

```bash
git clone <远程仓库路径> <本地路径>
```

说明：本地路径可以省略，会自动生成一个目录。

效果如图所示：

![image-20260705113136414](static/docs/Git使用教程/images/image-20260705113136414.png)

### 9. 从远程仓库抓取和拉取

- 抓取：将仓库里的更新都抓取到本地，但不进行合并，需额外使用`merge`命令合并分支；
  ...
  ![image-20260705114942617](static/docs/Git使用教程/images/image-20260705114942617.png)

  ![image-20260705115000771](static/docs/Git使用教程/images/image-20260705115000771.png)

- 拉取：将仓库里的更新都拉取到本地并进行合并，等同于`fetch+pull`。
  ...
  ![image-20260705115354769](static/docs/Git使用教程/images/image-20260705115354769.png)

说明：如果该分支前面配置了与远程分支的关联，则远端名称与分支名可以省略。

### 10. 冲突解决

在一段时间，A与B用户修改了同一个文件的同一行代码，此时会发生合并冲突。

A用户在本地修改代码后较先推送到远程仓库，此时B在本地修改代码后也要推送到远程仓库，B晚于A，此时B会出现如图所示情况，则是产生了冲突。

![image-20260705120508029](static/docs/Git使用教程/images/image-20260705120508029.png)

故需要先抓取远程仓库的提交到本地，经过合并解决冲突后才能推送到远程仓库，如图所示：

![Snipaste_2026-07-05_11-56-26](static/docs/Git使用教程/images/Snipaste_2026-07-05_11-56-26.png)

**远程分支也是一种分支，所以合并时冲突的解决方式也和解决本地分支冲突相同**，此处不再赘述。
...
### 1. windows无法创建以`.`开头的文件

这里以创建`.gitignore`为例，在对应目录打开git终端，使用如下命令创建文件：
...
![image-20260705142026385](static/docs/Git使用教程/images/image-20260705142026385.png)

### 2. windows下看不到隐藏的文件、

有时使用命令初始化了本地仓库，在windows中的文件夹中却看不到`.git`文件夹，此时只需打开此电脑，点击查看，将**隐藏的项目**勾选即可，如图所示：

![image-20260705142723016](static/docs/Git使用教程/images/image-20260705142723016.png)
...
- 远程操作类
  - git clone <远程仓库路径>    克隆仓库
  - git push <远端名称> <本地分支名>    推送提交到远程仓库
